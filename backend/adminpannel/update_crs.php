<?php
	...
	if(isset($_POST['flag'])&& $_POST['flag']=='update')
	{
		ob_clean();
		echo "in update";
		$sql_up="update `course` set `name` ='".$_POST['name']."',`fee`='".$_POST['fee']."',`duration`='".$_POST['duration']."',`time_id`='".$_POST['timing']."' where `id`=".$id;
		...
		{
			header('location:manage_course.php?msg=Not Updated');
		}
	}
...
<h1 style="font-size:40px;color: #006699;text-align:center"><b><u> UPDATE COURSE</u></h1>
		<div class="element">
						
				<?php
					$id=$_GET['id'];
					$sql_cour="select * from `course` where `id`='".$id."'";
				...
				?>
				 ...
				 <input type="text" name="name" value="<?php echo $update['name'];?>" class="name"pattern="^[a-zA-Z][a-zA-Z ]*$" title="please enter only alphabets" >
				 <?php }?>
		</div>
		...
		<input type="text"name="duration"value="<?php echo $update['duration'];?>" pattern="[0-9]+ [a-z]+" title="Enter valid format [number](days/months/years/weeks)">
		</div>
		<div class="element">
			...
			<select name="timing" id="timing" >
				...
				<?php
				  ...
				  {
					  $req=" ";
					  if($durta['id']==$update['time_id'])
					  {
						  $req="selected";
					  }
				?>
				  <option value="<?php echo $durta['id'];?>"<?php echo $req;?>><?php echo $durta['time'];?></option>	
				  <?php }?>
