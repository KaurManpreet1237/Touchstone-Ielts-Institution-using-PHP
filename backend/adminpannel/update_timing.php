<?php
	...
	if(isset($_POST['flag'])&& $_POST['flag']=='update')
	{
		ob_clean();
		echo "in update";
		$sql_up="update `timing` set `time` ='".$_POST['time']."' where `id`=".$id;
		echo "after update query";
		$count=update_data($sql_up);
		echo " update";
		if($count==0)
		{
			header('location:manage_timing.php?msg=Updated');
		}
		else
		{
			header('location:manage_timing.php?msg=Not Updated');
		}
	}
...
<h1 style="font-size:40px;color: #006699;text-align:center"><u><b> UPDATE TIMING</b></u></h1>
		<div class="element">
						
				<?php
					$id=$_GET['id'];
					$sql_cour="select * from `timing` where `id`='".$id."'";
				...
				?>
				 ...
				 <input type="text" name="time" value="<?php echo $update['time'];?>" >
				 <?php }?>
		</div>
		<div class="element">
