<?php
 ...
 ob_start();
  ...
  include_once('../../code/config.php');
	...
	{
		echo "inside flag";
		$sql_cour="select`edate1`,`edate2` from `exam_date` where `edate1`='".$_POST['name1']."'";
		$return=get_data($sql_cour);
		$count=$return->rowCount();
          if($count==0)
		  {
			  ob_clean();
			$sql_ins="insert into `exam_date`(`edate1`,`edate2`) values('".$_POST['name1']."','".$_POST['name2']."')";
            echo "after insert";
			
			$return=insert_data($sql_ins);
			echo "after insert data function";
			header('location:add-exam-date.php?msg=record inserted');
		  }
		  else
		   {
			   ob_clean();
			    header('location:add-exam-date.php?msg=record already exists');
		}
...
<h1  style="font-size:40px;color: #006699;text-align:center"><b><u>ADD EXAM DATES</u></b></h1>
    <div class="element">
      <label for="name" > DATE 1:</label>
      <input type="date" id="name" name="name1" >
    </div>
	
	 <div class="element">
      <label for="fee">DATE 2:</label>
      <input type="date" name="name2">
    </div>
	<div class="element">
				<input type="hidden" name="flag" id="flag" value="submit">
				<input type="submit" name="submit" id="submit" value="Add Exam Date">
				</div>
</div>
