...
if(isset($_POST['flag']) && $_POST['flag']=='submit')
{
	...
	echo $sql="insert into`achievements`(`description`,`doe`,`image`) values('".$_POST['des']."','".$_POST['doe']."','".$_FILES['pic']['name']."')";
	$result=insert_data($sql);
	if($result==1)
	{
		ob_clean();
		header('location:add_achievement.php?msg=Record Inserted Successfully');
		
	}
	else
	{
		ob_clean();
		header('location:add_achievement.php?msg=Record Not Inserted');
	}
}
?>
<html>
...
<div id="contentt">
	<div class="wrap-inn">
		<h2 style="font-size:40px;color: #006699;text-align:center"> <b><u> ADD ACHIEVEMENTS</u></b></h2>
			<form action="" method="POST" enctype="multipart/form-data" autocomplete>
				...
				<input type="submit" name="submit" id="submit" value="Add  ACHIEVEMENTS">
				</div>
			</form>
	</div>
</div>
